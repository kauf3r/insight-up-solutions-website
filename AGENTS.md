...
> **Canonical agent manual for every coding agent** (Claude Code, Codex, Cursor, etc.).
> `CLAUDE.md` beside this file is a one-line `@AGENTS.md` import stub; Claude Code >= 2.1.277
> reads `AGENTS.md` directly when no `CLAUDE.md` exists. Edit THIS file, never the stub.
...
- **RANGE is never named publicly.** the UAS test range (the UAS test range client) is anonymized as *"an FAA-designated UAS test range on the California coast"* in all site copy and case-study material until RANGE explicitly approves being named. See the gate block at the top of `docs/business/case-study-uas-test-range-DRAFT.md`.
...
1. **Naming the client.** Writes "the UAS test range" into site copy or the case study. → Rule: RANGE is anonymized as "an FAA-designated UAS test range on the California coast" everywhere public until RANGE approves. Both gates in the case-study draft header must clear first.
...
- [ ] RANGE anonymization intact; no `[ask Andy]` placeholder shipped
...
- Admin notifications go to `andy@andykaufman.net` (Andy's RANGE work address, hardcoded ×4 in `routes.ts`).
- Repo lives at `~/Desktop/Projects/insight-up-solutions` (iCloud root — slated to migrate to `~/dev`).

## Knowledge (AndyOS KB)

Andy's cross-business knowledge base (~4,800 pages: land investing, UAS/RANGE, AI tooling, clients, content, personal ops) lives at `~/dev/claude-life-os/LLM-context/` and is queryable from this repo via the `claude-life-os-kb` MCP server (registered in `.mcp.json`).
